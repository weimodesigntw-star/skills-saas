...
export const orderItemSchema = z.object({
  ...
  qty: z.coerce.number().int('數量需為整數').min(1, '數量需大於 0'),
  shipped_qty: z.coerce.number().int('已出需為整數').min(0).default(0),
  unit_price: z.coerce.number().int('單價需為整數').min(0, '單價不可為負'),
  discount_pct: z.coerce.number().int('折數需為整數').min(0).max(100).default(100),
